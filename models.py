...
class TextEncoder(nn.Module):
  ...
  def forward(self, x, x_lengths, prosody=None):
    x = (self.emb(x) + self.prosody_emb(prosody)) * math.sqrt(self.hidden_channels) # [b, t, h]
...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, x, x_lengths, y, y_lengths, sid=None, duration=None, prosody=None):

    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, prosody=prosody)
  ...
  def infer(self, x, x_lengths, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None, prosody=None):
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, prosody=prosody)
    if self.n_speakers > 1:
      g = self.emb_g(sid).unsqueeze(-1) # [b, h, 1]
    else:
