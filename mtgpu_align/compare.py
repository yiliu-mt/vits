...
for filename in os.listdir('mtgpu_align/cpu'):
    if not filename.endswith("_output.npy"):
        continue
    filenames.add(filename.rsplit('_', 1)[0])

for filename in filenames:
    print(f"Compare {filename}")
    ...
    np.allclose(cpu_enc_x, mt_enc_x, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_enc_m_p, mt_enc_m_p, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_enc_logs_p, mt_enc_logs_p, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_g, mt_g, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_dp_logw, mt_dp_logw, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_dp_w, mt_dp_w, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_dp_attn, mt_dp_attn, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_flow_m_p, mt_flow_m_p, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_flow_logs_p, mt_flow_logs_p, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_flow_z_p, mt_flow_z_p, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_flow_z, mt_flow_z, rtol=1e-6, atol=1e-4)
    np.allclose(cpu_output, mt_output, rtol=1e-6, atol=1e-4)
