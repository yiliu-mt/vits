#!/usr/bin/env python3
# coding=utf-8
# copy from https://github.com/speechio/chinese_text_normalization/tree/master/python
...
class Link:
    ...
    处理URL的链接类
    """
    def __init__(self, link=None, chntext=None):
        self.link = link
        self.chntext = chntext
        self._symbol_map = {
            ':': '冒号',
        ...
        # self.pattern = re.escape(self._symbol_map.keys)
        # self.repl = lambda m: self._symbol_map.get(m.group(0))

    def link2chntext(self):
        text = self.link
        # re.sub(pattern, repl, self.repl, text)

        # 标点规范化
        lst = re.split(r'([^a-zA-Z0-9])', text)
        lst = [x for x in lst if x]
        # print('lst:', lst)

        short_pause = '、'
        for i, x in enumerate(lst):
            if x in self._symbol_map:
                lst[i] = self._symbol_map.get(x)

        # print('lst:', lst)
        text = short_pause.join(lst)
...
class Preprocess:
    ...
    default_rule_file = os.path.join(os.path.dirname(__file__), 'rules.tsv')

    def __init__(self, text=None, chntext=None, rule_file=None):
        self.text = text
        self.rules = None

        if rule_file is None:
            rule_file = self.default_rule_file

        if not os.path.isfile(rule_file):
            print('[WARNING] preprocess will not work,'
                  ' because rule file {} not found.'.format(rule_file))
            return
        ...
        with open(rule_file) as f:
            for line in f:
                ...
                _ = re.compile(re.escape(b))  # for check if regex valid
                rules.append([pattern, b])
        self.rules = rules

    def text2chntext(self):
        if not self.rules:
            return self.text
...
def normalize_nsw(raw_text):
    # 预处理
    raw_text = Preprocess(raw_text).text2chntext()
    ...
    if matchers:
        #print('link')
        for matcher in matchers:
            text = text.replace(matcher[0], Link(link=matcher[0]).link2chntext(), 1)
...
class TextNorm:
    def __init__(self,
        ...
        assert not ((not self.remove_punc) and check_chars), "cannot check chars when use in keep-punc mode"
